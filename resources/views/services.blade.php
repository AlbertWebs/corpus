<div class="case8-section-area sp1 " id="service">
    <div class="container">
       <div class="row align-items-center">
          <div class="col-lg-4">
             <div class="case-header heading13">
                <h5 data-aos="fade-up" data-aos-duration="1000">What We Do</h5>
                <h2 class="text-anime-style-3 theme-text">Our Services</h2>
                <p data-aos="fade-up" data-aos-duration="1000">We believe in the power of clear goals, strategic <br class="d-md-block d-none"> planning, and relentless execution to turn visions into reality.</p>
                <br>
                <div class="btn-area1" data-aos="fade-up" data-aos-duration="1200">
                   <a href="#why" class="header-btn15">Why Choose Us <span><i class="fa-solid fa-arrow-right"></i></span></a>
                </div>
             </div>
          </div>
          <div class="col-lg-8">
             <div class="case-study-sliderarea owl-carousel">
                <div class="author-case-area">
                   <div class="img1 reveal image-anime">
                      <img class="service-image" src="{{url('/')}}/uploads/img101-modified.jpg" alt="">
                   </div>
                   <div class="content-area">
                      <div class="content">
                         <a href="#">Businesses</a>
                         <div class="space16"></div>
                         <p>
                            Strategy formulation, HR Audit,Business Advisory,Audit Services,Tax advisory,Financial planning services,Corporate Governance,Sustainability Consulting, Trainings
                         </p>
                      </div>
                      <div class="arrow">
                         <a href="#"><i class="fa-solid fa-arrow-right"></i></a>
                      </div>
                   </div>
                </div>
                <div class="author-case-area">
                   <div class="img1 reveal image-anime">
                      <img class="service-image" src="{{url('/')}}/uploads/img112-modified.jpg" alt="">
                   </div>
                   <div class="content-area">
                      <div class="content">
                         <a href="#">Individuals</a>
                         <div class="space16"></div>
                         <p>
                            Personal finance, investment consultation, wealth management, retirement planning
                         </p>
                      </div>
                      <div class="arrow">
                         <a href="#"><i class="fa-solid fa-arrow-right"></i></a>
                      </div>
                   </div>
                </div>
                <div class="author-case-area">
                   <div class="img1 reveal image-anime">
                      <img class="service-image" src="{{url('/')}}/uploads/img45.jpg" alt="">
                   </div>
                   <div class="content-area">
                      <div class="content">
                         <a href="#">Family Businesses</a>
                         <div class="space16"></div>
                         <p>Succession planning, Wealth Management, Family Business Constitution,Business Advisory, Portfolio Management, Tax Advisory, Audit Services,Project Management.
                         </p>
                      </div>
                      <div class="arrow">
                         <a href="#"><i class="fa-solid fa-arrow-right"></i></a>
                      </div>
                   </div>
                </div>

                <div class="author-case-area">
                   <div class="img1 reveal image-anime">
                      <img class="service-image" src="{{url('/')}}/uploads/29249842033_15cc240716_b-modified.jpg" alt="">
                   </div>
                   <div class="content-area">
                      <div class="content">
                         <a href="#">Investment Groups(Chamas)- </a>
                         <div class="space16"></div>
                         <p>Financial advisory services for groups provide expert guidance on budgeting, investments, and risk management to enhance financial stability and growth.</p>
                      </div>
                      <div class="arrow">
                         <a href="#"><i class="fa-solid fa-arrow-right"></i></a>
                      </div>
                   </div>
                </div>
             </div>
          </div>
       </div>
    </div>
 </div>
